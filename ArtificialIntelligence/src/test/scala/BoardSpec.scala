...
class BoardSpec extends UnitSpec {
  ...
  "A Board" should "spawn pieces in the right position" in {
    val board = Board.fromJson(BoardSpec.BOARD_OF_WIDTH_FOUR_JSON)
    board.startNewGame()
    board.activeBlock.transformedCells should be (Array(HexCell.fromXY(1, 0)))
  }
